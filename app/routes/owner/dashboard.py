...
def dashboard():
    ...
    if period not in (7, 30, 90):
        period = 30
    case_bucket = request.args.get("case_bucket", "month").strip()
    if case_bucket not in ("week", "month", "quarter"):
        case_bucket = "month"
    wheel_bucket = request.args.get("wheel_bucket", "month").strip()
    if wheel_bucket not in ("week", "month", "quarter"):
        wheel_bucket = "month"
    active_engagement_tab = request.args.get("engagement_tab", "wheel").strip()
    if active_engagement_tab not in ("wheel", "cases", "missions"):
        active_engagement_tab = "wheel"
    ...
    case_openings_period_chart = get_case_openings_period_chart(int(club_id), case_bucket)
    wheel_spins_period_chart = get_wheel_spins_period_chart(int(club_id), wheel_bucket)
    ...
    return render_template(
        ...
        case_openings_period_chart=case_openings_period_chart,
        wheel_spins_period_chart=wheel_spins_period_chart,
        mission_completions_chart=mission_completions_chart,
        first_visit_feedback=first_visit_feedback,
        selected_period=period,
        selected_case_bucket=case_bucket,
        selected_wheel_bucket=wheel_bucket,
        active_engagement_tab=active_engagement_tab,
    )
