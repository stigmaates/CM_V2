(() => {
  ...
  async function openGuest(id){
    ...
    try{
      ...
      const r=data.guest,h=r.health,v=r.value,e=r.engagement,o=r.overall||{},f=r.visits,g=r.games||{},pattern=r.visit_pattern||{},p30=v.percentiles_30d||{}; $('gpGuestName').textContent=r.name;
      ...
      $('gpGuestDetail').innerHTML=`<section class="gp-guest-summary"><div class="gp-guest-meta"><span>ID ${esc(r.guest_id)}</span><span>${esc(r.phone||'Номер не указан')}</span><span class="${r.has_telegram?'is-connected':'is-muted'}">${r.has_telegram?'Telegram подключён':'Без Telegram'}</span><span>Последний визит: ${esc(date(f.last_visit_date))}</span></div>${detailTags([r.lifecycle_label,r.audience_label,pattern.period,pattern.calendar,...(r.segments||[])])}</section>
        ${deviations.length?`<div class="gp-detail-note gp-detail-alert"><b>Причина попадания в отклонения</b>${deviations.map(change).join('')}</div>`:''}
        <section class="gp-overall-card"><div><span>Общий балл гостя</span><strong>${num(o.score)}${o.score==null?'':'<small> / 100</small>'}</strong><b>${esc(o.label||'Недостаточно данных')}</b></div></section>
        <div class="gp-detail-scores">
          <section class="gp-detail-card gp-health-card"><header><span class="gp-detail-letter">П</span><div><h3>Посещения</h3><small>Привычка посещений</small></div><strong>${num(h.score)}</strong></header>${metricBars([['Давность',h.recency],['Частота',h.frequency],['Тренд',h.trend],['Регулярность',h.consistency]])}<div class="gp-detail-snapshots"><span><small>7 дней назад</small><b>${num(h.score_7d_ago)}</b></span><span><small>14 дней назад</small><b>${num(h.score_14d_ago)}</b></span><span><small>30 дней назад</small><b>${num(h.score_30d_ago)}</b></span></div></section>
          <section class="gp-detail-card gp-value-card"><header><span class="gp-detail-letter">Ц</span><div><h3>Ценность</h3><small>Все показатели за 30 дней</small></div><strong>${num(v.score)}</strong></header>${dl([['Пополнения',num(v.revenue_30d)+' ₽'],['Игровые часы',num(v.played_hours_30d)+' ч'],['Визиты',num(v.visits_30d)],['Пополнения на визит',num(v.avg_check_30d)+' ₽'],['Выше гостей по пополнениям',p30.revenue_30d==null?'—':num(p30.revenue_30d)+'%'],['Выше гостей по часам',p30.played_hours_30d==null?'—':num(p30.played_hours_30d)+'%'],['Выше гостей по визитам',p30.visits_30d==null?'—':num(p30.visits_30d)+'%'],['База сравнения',num(v.reference_count_30d)]])}</section>
          <section class="gp-detail-card gp-engagement-card"><header><span class="gp-detail-letter">В</span><div><h3>Вовлечённость</h3><small>Активность в Кибер Бонус</small></div><strong>${num(e.score)}</strong></header>${dl([['Telegram',r.has_telegram?'Подключён':'Не подключён'],['Задания · 30 дней',num(e.missions_completed_30d)],['Контракты выбраны · 30 дней',num(e.contracts_selected_30d)],['Контракты выполнены · 30 дней',num(e.contracts_completed_30d)],['Все действия · 30 дней',num(e.cb_actions_30d)],['Дней активности подряд',num(e.current_streak)],['Последняя активность',date(e.last_cb_activity_at)]])}</section>
        </div>
        <section class="gp-guest-context"><div><h3>О госте</h3>${dl([['Состояние посещений',h.reason_text||'—'],['Обычный интервал',f.typical_gap_days==null?'—':num(f.typical_gap_days)+' дн.'],['Всего визитов',num(f.visits_total)],['Ночные визиты',pattern.night_share==null?'—':num(pattern.night_share)+'%'],['Визиты в выходные',pattern.weekend_share==null?'—':num(pattern.weekend_share)+'%']])}</div>${g.favorite_game||g.recent_game_14d?`<div><h3>Игровой профиль</h3>${dl([['Любимая игра',gameValue(g.favorite_game,g.favorite_game_hours)],['За последние 2 недели',gameValue(g.recent_game_14d,g.recent_game_14d_hours)],['Данные обновлены',date(g.updated_at)]])}</div>`:''}</section>
