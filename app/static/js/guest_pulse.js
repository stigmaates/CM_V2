(() => {
  ...
  const person = (r) => `<button class="gp-person" type="button" data-guest="${r.guest_id}">${esc(r.name)}<span>${esc(r.lifecycle_label)}</span></button>`;
  ...
  function render(data){
    ...
    $('gpTotal').textContent=num(data.total);$('gpSelectionCount').textContent=`Выбрано гостей: ${num(data.selected_count)}`;
    $('gpMail').disabled=!data.selected_count;$('gpInteract').disabled=!data.selected_count;
    $('gpAudienceList').innerHTML=data.audiences.map(a=>`<button type="button" class="gp-audience ${filters.audience_type===a.key?'is-active':''}" data-audience="${a.key}" aria-pressed="${filters.audience_type===a.key}"><span class="gp-label"><i class="gp-dot" style="background:${a.color}"></i>${esc(a.label)}</span><strong>${num(a.count)} <small>/ ${num(a.total)}</small></strong></button>`).join('');
    const svg=$('gpChart');svg.replaceChildren();let offset=0;const circumference=2*Math.PI*91;
    function circle(color){const el=document.createElementNS('http://www.w3.org/2000/svg','circle');for(const [k,v] of Object.entries({cx:120,cy:120,r:91,fill:'none',stroke:color,'stroke-width':27}))el.setAttribute(k,v);return el;}
    svg.append(circle('#332243'));
    data.audiences.filter(a=>a.count>0).forEach(a=>{
      const sector=circle(a.color),length=circumference*a.count/data.total,tip=`${a.label}: ${num(a.count)} · ${num(a.count/data.total*100)}%`;
      sector.classList.add('gp-chart-sector');sector.setAttribute('stroke-dasharray',`${length} ${circumference-length}`);sector.setAttribute('stroke-dashoffset',-offset);sector.setAttribute('transform','rotate(-90 120 120)');sector.setAttribute('tabindex','0');sector.setAttribute('role','button');sector.setAttribute('aria-label',tip);
      if(filters.audience_type && filters.audience_type!==a.key)sector.style.opacity='.35';
      sector.addEventListener('mouseenter',()=>{$('gpChartTip').textContent=tip;});sector.addEventListener('focus',()=>{$('gpChartTip').textContent=tip;});
      sector.addEventListener('click',()=>choose(a.key));sector.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();choose(a.key);}});svg.append(sector);offset+=length;
    });
    ...
    $('gpGuestsCount').textContent=`${num(data.selected_count)} гостей · показано до ${data.page_size} на странице`;
    ...
    $('gpDeviationCount').textContent=`${num(data.deviation_count)} гостей с отклонениями`;$('gpDeviationInteract').disabled=!data.deviation_count;
  ...
  async function load(){
    ...
    error('');$('gpMail').disabled=true;$('gpInteract').disabled=true;$('gpDeviationInteract').disabled=true;
    try{const data=await api('/owner/api/guest-pulse?'+new URLSearchParams({...filters,page,deviation_page:deviationPage}),{signal:own.signal});if(own===controller)render(data);}
  ...
  document.querySelectorAll('[data-key]').forEach(input=>input.addEventListener('input',()=>{
    ...
    page=1;deviationPage=1;controller?.abort();$('gpMail').disabled=true;$('gpInteract').disabled=true;$('gpDeviationInteract').disabled=true;clearTimeout(timer);timer=setTimeout(load,250);
  }));
  ...
  async function handoff(mode){
    ...
    catch(e){error(e.message);$('gpGuestDialog').close();buttons.forEach(b=>b.disabled=false);}
  }
  ...
  async function openGuest(id){
    selectedGuest=Number(id);detailController?.abort();detailController=new AbortController();const own=detailController;
    ...
    try{
      ...
      $('gpGuestDetail').innerHTML=`<div class="gp-detail-status">${esc(r.lifecycle_label)} · ${esc(r.phone||'Номер не указан')}</div>${deviations.length?`<div class="gp-detail-note">Причина попадания в отклонения:${deviations.map(change).join('')}</div>`:''}
        ...
        <section class="gp-detail-card"><h3>Engagement</h3><strong>${num(e.score)}</strong><small>Активность в Кибер Бонус</small>${dl([['Telegram',e.telegram_connected?'Подключён':'Не подключён'],['Задания · 30 дней',num(e.missions_completed_30d)],['Игровые действия · 30 дней',num(e.cb_actions_30d)],['Дней активности подряд',num(e.current_streak)],['Последняя активность',date(e.last_cb_activity_at)]])}</section></div>
      ...
      $('gpGuestInteract').disabled=false;
