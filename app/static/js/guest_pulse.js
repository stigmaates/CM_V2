(() => {
  ...
  async function openGuest(id){
    ...
    try{
      ...
      const r=data.guest,h=r.health,v=r.value,e=r.engagement,o=r.overall||{},f=r.visits; $('gpGuestName').textContent=r.name;
      ...
      $('gpGuestDetail').innerHTML=`<div class="gp-detail-status">${esc(r.lifecycle_label)} · ${esc(r.phone||'Номер не указан')}</div>${deviations.length?`<div class="gp-detail-note">Причина попадания в отклонения:${deviations.map(change).join('')}</div>`:''}
        <section class="gp-overall-card"><div><span>Общий балл гостя</span><strong>${num(o.score)}${o.score==null?'':'<small> / 100</small>'}</strong><b>${esc(o.label||'Недостаточно данных')}</b></div><p>${o.score==null?'Балл рассчитывается, когда есть все три оценки: П, Ц и В.':`П ${num(h.score)} × 35% + Ц ${num(v.score)} × 40% + В ${num(e.score)} × 25%`}</p></section>
