...
def _normalize_activity_bucket(bucket: str) -> str:
    if bucket in _ACTIVITY_BUCKET_LABELS:
        return bucket
    return "month"


def _activity_period_sql(field_name: str, bucket: str) -> str:
    if bucket == "week":
        return f"DATE_SUB(DATE({field_name}), INTERVAL WEEKDAY({field_name}) DAY)"
    if bucket == "quarter":
        return (
            "STR_TO_DATE("
...
def _add_period(value, bucket: str):
    if bucket == "week":
        return value + timedelta(days=7)

    months_to_add = 3 if bucket == "quarter" else 1
...
def _format_activity_period(value, bucket: str) -> str:
    if bucket == "week":
...
def _build_period_axis(period_starts, bucket: str) -> list:
    clean_starts = sorted({_normalize_period_start(value) for value in period_starts if value})
    if not clean_starts:
        return []

    axis = []
    current = clean_starts[0]
    last = clean_starts[-1]
    while current <= last:
        axis.append(current)
        current = _add_period(current, bucket)
    return axis


def get_case_openings_period_chart(club_id: int, bucket: str = "month", top_cases: int = 6) -> dict:
    """Return grouped case opening dynamics by week, month, or quarter."""
    bucket = _normalize_activity_bucket(bucket)
    if not club_id:
        return {
            "bucket": bucket,
            "bucket_label": _ACTIVITY_BUCKET_LABELS[bucket],
            "labels": [],
            "series": [],
            "max_value": 0,
        }

    period_expr = _activity_period_sql("o.created_at", bucket)
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                SELECT
                    ...
                    o.case_id,
                    COALESCE(c.name, CONCAT('Кейс #', o.case_id)) AS case_name,
                    COUNT(*) AS openings_count
                FROM guest_case_openings o
                LEFT JOIN club_cases c
                  ON c.club_id = o.club_id
                 AND c.id = o.case_id
                WHERE o.club_id = %s
                GROUP BY period_start, o.case_id, case_name
                ORDER BY period_start ASC, openings_count DESC, case_name ASC
                """,
                (club_id,),
            )
            rows = cursor.fetchall() or []
    ...
    period_axis = _build_period_axis((row.get("period_start") for row in rows), bucket)
    if not period_axis:
        return {
            "bucket": bucket,
            "bucket_label": _ACTIVITY_BUCKET_LABELS[bucket],
    ...
    period_case_counts = defaultdict(lambda: defaultdict(int))
    for row in rows:
        period_start = _normalize_period_start(row.get("period_start"))
        case_name = row.get("case_name") or f"Кейс #{row.get('case_id') or ''}".strip()
        count = int(row.get("openings_count") or 0)
        case_totals[case_name] += count
        period_case_counts[period_start][case_name] += count

    top_case_names = [
        case_name
        for case_name, _count in sorted(case_totals.items(), key=lambda item: (-item[1], item[0]))[: max(top_cases, 1)]
    ]
    include_other = len(case_totals) > len(top_case_names)
    series_names = top_case_names + (["Другие"] if include_other else [])
    series = []
    max_value = 0
    for index, case_name in enumerate(series_names):
        values = []
        for period_start in period_axis:
            period_counts = period_case_counts.get(period_start, {})
            if case_name == "Другие":
                value = sum(count for name, count in period_counts.items() if name not in top_case_names)
            else:
                value = period_counts.get(case_name, 0)
            values.append(value)
        ...
        series.append(
            {
                "name": case_name,
                "values": values,
                "color": _CASE_PERIOD_COLORS[index % len(_CASE_PERIOD_COLORS)],
                "total": sum(values),
            }
        )
    ...
    return {
        "bucket": bucket,
        "bucket_label": _ACTIVITY_BUCKET_LABELS[bucket],
        "labels": [_format_activity_period(period_start, bucket) for period_start in period_axis],
        "series": series,
        "max_value": max_value,
    }


def get_wheel_spins_period_chart(club_id: int, bucket: str = "month") -> dict:
    """Return wheel spin dynamics by week, month, or quarter."""
    bucket = _normalize_activity_bucket(bucket)
    if not club_id:
        return {
            "bucket": bucket,
    ...
    period_expr = _activity_period_sql("created_at", bucket)
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            cursor.execute(
    ...
    period_axis = _build_period_axis((row.get("period_start") for row in rows), bucket)
    counts_by_period = {
        _normalize_period_start(row.get("period_start")): int(row.get("spins_count") or 0)
        for row in rows
        if row.get("period_start")
    }
    values = [counts_by_period.get(period_start, 0) for period_start in period_axis]
    ...
    return {
        ...
        "items": [
            {
                "label": _format_activity_period(period_start, bucket),
                "count": count,
                "height": round((count / max_value) * 100, 1) if max_value else 0,
            }
            for period_start, count in zip(period_axis, values)
        ],
        "max_value": max_value,
        "total": sum(values),
    }
