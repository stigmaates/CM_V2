...
2. Добавить в `/root/cm_stage/CM_V2/.env`:
   ...
   # Создать Server-side API key в https://developers.faceit.com/
...
- «Последние матчи» в карточке Counter-Strike 2 находит FACEIT-профиль по
  привязанному SteamID64 и загружает пять последних матчей FACEIT. В окне
  показываются карта, результат, тип матча и K/D/A. Матчи Premier и обычного
  матчмейкинга Steam сюда не входят.
- `FACEIT_API_KEY` используется только сервером и не передаётся в браузер.
